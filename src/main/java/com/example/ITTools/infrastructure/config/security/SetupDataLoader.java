  ...
  ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if (userRepository.findByUsername("admin@test.com").isEmpty()) {
            ...
            user.setUsername("admin@test.com"); // Establece el correo electrónico aquí
            user.setPassword(passwordEncoder.encode("test")); // Establece la contraseña encriptada aquí
            user.setAuthorities(setRole);
            user.setEnabled(true); // Asegúrate de establecer el estado habilitado
